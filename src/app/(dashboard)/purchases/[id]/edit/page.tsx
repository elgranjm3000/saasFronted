...
const EditPurchasePage = () => {
  ...
  const [originalStatus, setOriginalStatus] = useState<'pending' | 'approved' | 'received' | 'cancelled'>('pending');
  ...
  const purchaseId = params?.id ? Number(params.id) : null;
  ...
  const fetchPurchase = async () => {
    ...
    try {
      ...
      console.log('Purchase data from API:', purchase);

      // Handle missing items array - backend may not return it
      const items = purchase.items || [];

      // Fetch supplier details if not included
      let supplier = suppliers.find(s => s.id === purchase.supplier_id);
      if (!supplier && purchase.supplier_id) {
        try {
      ...
      let warehouse = warehouses.find(w => w.id === purchase.warehouse_id);
      if (!warehouse && purchase.warehouse_id) {
        try {
      ...
      setFormData({
        ...
        expected_delivery_date: purchase.expected_delivery_date ? new Date(purchase.expected_delivery_date).toISOString().split('T')[0] : '',
        items: itemsWithNames,
        notes: purchase.notes || ''
      });

      setOriginalStatus(purchase.status || 'pending');
      ...
      // If warehouse is set, load its products
      if (purchase.warehouse_id) {
        await fetchProductsForWarehouse(purchase.warehouse_id);
      }
    } catch (error) {
      console.error('Error fetching purchase:', error);
      setErrors({ general: 'Error al cargar la compra' });
    } finally {
  ...
  const updateProductStock = async (productId: number, warehouseId: number, quantityReceived: number) => {
    try {
      ...
      console.log(`Stock increased: Product ${productId}, Warehouse ${warehouseId}, New quantity: ${newQuantity}`);
    } catch (error) {
      console.error('Error updating product stock:', error);
      // Don't throw error, just log it - purchase update should still succeed
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!purchaseId) {
      setErrors({ general: 'ID de compra no válido' });
      return;
    }
    ...
    try {
      ...
      console.log('Updating purchase with data:', submitData);

      await purchasesAPI.update(purchaseId, submitData);

      // If status changed to "received" and wasn't before, increase stock
      if (formData.status === 'received' && originalStatus !== 'received' && formData.warehouse_id) {
        console.log('Status changed to received, updating stock...');
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div>
            <h1 className="text-3xl font-light text-gray-900">Editar Compra</h1>
            <p className="text-gray-500 font-light">Modifica la información de la compra</p>
          </div>
        </div>
      </div>
      ...
      {success ? (
        <div className="bg-green-50 border border-green-200 rounded-3xl p-12 text-center">
          ...
          <h2 className="text-2xl font-light text-green-900 mb-2">¡Compra actualizada!</h2>
          <p className="text-green-700 mb-6">La orden de compra se ha actualizado correctamente.</p>
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-gray-100 p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {formData.status === 'received' && originalStatus !== 'received' && (
              ...
              {formData.status === 'received' && originalStatus !== 'received' && (
                <div className="bg-green-50 border border-green-200 rounded-3xl p-6">
                  ...
                  <p className="text-sm text-green-800">
                    Al guardar, el stock de todos los productos se aumentará en sus respectivas cantidades.
                  </p>
                </div>
              )}
